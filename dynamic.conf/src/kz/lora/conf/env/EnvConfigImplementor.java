...
import kz.lora.conf.error.CannotConvertToType;
import kz.lora.conf.hot.Description;
import kz.lora.conf.core.util.GetAndToLen;
import kz.lora.conf.core.util.SplitToList;
import kz.lora.conf.core.util.StrToLen;
...
public class EnvConfigImplementor {
  ...
  private static class Param {
    ...
    public void init() {
      ...
      try {
        ...
        errors.addAll(SplitToList.splitToList((e.getMessage())));
        if (e.getCause() != null) {
          errors.addAll(SplitToList.splitToList(e.getCause().getClass().getSimpleName() + " : " + e.getCause().getMessage()));
        }
      }
  ...
  public static <T> T impl(Class<T> configInterface, EnvSource envSource) {
    ...
    if (params.stream().anyMatch(Param::hasErrors)) {
      ...
      err.append("    ").append(StrToLen.strToLen(head1, col1len)).append(' ');
      err.append("    ").append(StrToLen.strToLen(head2, col2len)).append(' ');
      err.append("    ").append(StrToLen.strToLen(head3, col3len)).append(' ');
      err.append("    ").append(StrToLen.strToLen(head4, col4len)).append('\n');
      ...
      for (Param param : params) {
        ...
        for (int i = 0; i < rows; i++) {
          ...
          err.append("    ").append(GetAndToLen.getAndToLen(col1, i, col1len)).append(' ');
          err.append("    ").append(GetAndToLen.getAndToLen(col2, i, col2len)).append(' ');
          err.append("    ").append(GetAndToLen.getAndToLen(col3, i, col3len)).append(' ');
          err.append("    ").append(GetAndToLen.getAndToLen(col4, i, col4len)).append('\n');
